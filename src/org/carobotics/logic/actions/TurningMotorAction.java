 ...
 * @author CaRobotics
 */
public class TurningMotorAction extends Action {
     ...
     * @param distance Linear distance (feet) to travel (positive is turn right, negative is turn left)
     ...
     * @param waitForDependenciesPeriod Period for the thread when it is waiting
     * for its dependencies to be satisfied
     * @param runningPeriod Period for the thread when it is actively running
     */
    public TurningMotorAction(TankDriveBase driveBase, double speedRPM, double distance,
        ...
        if (this.targetDistance < 0) {
            ...
            rightMotorInfo.reverseDirection = false;
        } else {
            leftMotorInfo.reverseDirection = false;
    ...
    private void findTargetRotations() {
        leftMotorInfo.targetRotations = leftJaguar.getPosition() + (targetDistance / wheelCircumference) * wheelCircumference * (1.0 / gearRatio);
        rightMotorInfo.targetRotations = rightJaguar.getPosition() + (-targetDistance / wheelCircumference) * wheelCircumference * (1.0 / gearRatio);
        errorMarginRotations = Math.abs(errorMarginDistance / wheelCircumference) * wheelCircumference * gearRatio;
        System.out.println("[TurnDistanceAction] Current Position: L - " + leftJaguar.getPosition() + " R - " + rightJaguar.getPosition() + " | Target Rotations: L - " + leftMotorInfo.targetRotations + " R - " + rightMotorInfo.targetRotations + " | Error Margin: " + errorMarginRotations);
    }

    public boolean isComplete() {
        return leftMotorInfo.isComplete || rightMotorInfo.isComplete;
    }
    ...
    protected void cycle() {
        ...
        if (System.currentTimeMillis() - leftMotorInfo.lastPositionCheckTime > 300) {
            ...
            if (rotationsToTarget > prevRotationsToTarget) {
                System.out.println("[TurnDistanceAction] [ATTN] Switching direction of left jaguar!");
            ...
            if (rotationsToTarget > prevRotationsToTarget) {
                System.out.println("[TurnDistanceAction] [ATTN] Switching direction of right jaguar!");
        ...
        System.out.println("[TurnDistanceAction] Jaguar position: L - " + leftJaguar.getPosition() + " R - " + rightJaguar.getPosition());
        ...
        if (isComplete()) {
            ...
            stopRunning();
            System.out.println("[TurnDistanceAction] Complete at position L - " + leftJaguar.getPosition() + " R - " + rightJaguar.getPosition());
            return;
        }
